...
仓库的 V1-A 工程基线和 V1-B 计算内核已经用户验收。当前没有活动区块；V1-C 仍为锁定的下一候选区块。会话状态、本地存储、真实产品页面、云环境和部署配置尚未实施。
...
| `apps/client/miniprogram/` | 可由微信开发者工具编译的最小小程序壳；`pages/bootstrap/` 只说明 V1-A 已就绪，不实现产品能力 |
...
- V1-C 及后续区块能力仍受区块门禁约束；不得借共享路径提前实现会话、存储、组件或页面范围。

## 规划但尚未创建或尚未实现

以下是跨多个区块的规划结构。当前没有活动区块，不实施这些锁定能力；已有文件只允许为修复已批准基线缺陷或维护治理事实而修改：
