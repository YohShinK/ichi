...
用户主动资料编辑继续复用 `pages/home`、`platform/cloud-account.ts` 与 `bind-wechat-profile`：nickname-only 直接走 owner-scoped 服务端安全审核；avatar edit 把 `chooseAvatar` 的本机临时 path 上传到 `profile-avatars/` 后只提交稳定 fileID，DB 更新成功后才清理旧头像。本地 `USER_DATA_PATH` 缓存仍以 `avatarFileId` 为一致性键。没有新增 collection、Guest／Anonymous 模型、migration 或默认头像副本；Local Board／Upload Submission／Current Cloud Publication、PHOTO、quota、R2 与 Storage/ACL 架构未改。

2026-08-30 V1.0.0 architecture closure：微信小程序已正式发布，线上源码固定为 `03942f2067959a4b8b0eb6223c949e51e768587d`。Local Board／Upload Submission／Current Cloud Publication 三层模型及 7 条产品规则均已实现、真机验证并 `FROZEN / CLOSED`；P1 保持轻量 published-version pointer，完整 snapshot/events 留在 versioned submission；冻结 R2 三 SHA、Storage CUSTOM、`recognition-temp/ Days=1` 与 13 个私有集合边界不变。当前没有活动区块，V1 active plans=`0`；V1.0.1 和 V2 仅为 backlog。

2026-08-29 V1 PHOTO fresh failure / P1 publication regression 收口：真机最新 `record_b329…23a1 / board-…d999 / v3` 已只读锁定为 fresh `PROVIDER_FAILED`；Provider HTTP 200、response parse、结构化五票和 AJV 全部成功，machine code 仍为 generic provider failure。结合确定性后续链（normalize 异常会保留专用 schema code、note safety 自身 fail closed）和历史同类生产事实，首个失败点收敛为 `APPROVED` publication transaction：Current Publication 改造重新把大型 `finalSnapshot + authoritativeDrawEvents` 复制进 P1，重现 CloudBase `-502001` 风险。P1 现恢复为小型发布指针，只原子写 `publishedSubmissionVersion` 与批准状态；完整权威内容继续归版本化 `drawSubmissions`，`getMyRecords` 精确读取 published version 并投影 snapshot、events、note 与 location，同时保留更新失败 attempt 的状态用于“核验异常”与同版本重试。诊断 checkpoint 延伸到 `NORMALIZED / RECONCILED / NOTE_REVIEWED / PUBLICATION_TRANSACTION_STARTED / RESULT_PERSISTED`，错误码兼容 `code/errCode`。生产只以 code update 发布 `recognize-draw-tickets` 和 `get-my-records`，环境键未被覆盖；反向树 hash 分别为 `7095d04d795694736f6264ef677a96e3c47a64f24058d38723075a1bdc93397b` 与 `96b059ffa5a80b1d354732859310311eb8ba7f743389811a33ce8b92d64702ff`。

2026-08-29 V1 PHOTO provider failure closure：生产最近两块不同 Local Board 的六次 submission 均已保存正确的 `verificationId=recordId+boardId+submissionVersion`、当前版本 Storage fileID、LOCATION PASS 与五条 `A/B/B/C/D` authoritative events，但 Provider 技术异常被 `verify-prize-tickets` catch 错写为终态 `PHOTO_FAILED`，同时清空并即时删除 evidence。`verify-prize-tickets/index.js` 现将 claimed technical failure 持久化为可重试 `PROVIDER_FAILED`，保留同一版本 `imageFileId`，并保存不含 raw payload 的 provider stage、HTTP status、request id 与 AJV/票数诊断；同版本 verify 已有读取 `PROVIDER_FAILED` 的恢复分支，既有 50 分钟 deletion job 仍负责最终过期清理。Provider 成功后的 `observedFromTickets + exactReconcile` 和真正 `PHOTO_FAILED` 路径不变。生产只部署 `recognize-draw-tickets`；首次 code deploy 将配置文件中唯一显式半径误当完整环境覆盖，反向检查立即发现并从部署前只读快照恢复原 6 键，仓库未保存密钥。最终函数 Active/Available、Nodejs20.19、`index.main`、layer v1，反向代码树与本地 7 文件 hash 均为 `54285603ac7be8072bc822bca06efdf2b93d5b8961ea8f41eb66a839bbc3d4e1`。

2026-08-29 V1 DRAW SESSION EXIT blocker：`pages/home/index.ts` 的 draw 工作台继续由 `currentView="draw" + activeDraft.boardId` 控制，Local Board 的 `history` 是上传 authoritative draw batch 的唯一客户端事实源。真实退出不是进程 crash 或 navigation：`onShow → refreshCloudAccount → refreshCloudRecords → resumePendingCloudPublicationDeletions` 的晚到结果即使没有 pending deletion，也曾以 `refreshDrafts(null)` 清空 active identity，随后 `refreshDrafts` 的既有 fallback 把 draw view 改成 start/import。恢复链现只用 `refreshDrafts()` 重新投影 repository，保留当前 active board；ticket verification、赏票重传与备注审核的晚到完成回调同样不得用 null active identity 覆盖用户后来重新进入的 draw session。逐抽仍先写页面投影并在 0ms persistence task 中同步 `draftRepository.upsert`，本轮没有改变 draw 算法、submission boundary 或服务端 PHOTO 合同。

2026-08-28 V1 LOCAL BOARD / UPLOAD SUBMISSION / CURRENT CLOUD PUBLICATION 架构收口：`local-draw-drafts.ts` 以 `boardId` 保存 B1，并新增可恢复的 `pendingCloudDeletion`；`verify-prize-tickets/index.js` 将 S1/S2…全部隔离在 `drawSubmissions`，失败 attempt 不再提前改写 `observationCandidates`；APPROVED 事务用单调 `publishedSubmissionVersion` 防止迟到旧版本覆盖，并更新同一稳定 P1 `recordId`。`runtime.js/getMyRecords` 过滤 deleting、superseded 和显式删除墓碑后，按 `ownerAccountId + boardId` 选出唯一可信 current publication；`cloud-records.ts` 只显示 P1 的已批准快照，不把失败／pending attempt 的 snapshot 当作当前发布。

`pages/home/index.ts` 的 MY_RECORDS 仍为 B1-only；MY_UPLOADS 从 current-only P1 列表投影，并在客户端按 `boardId` 防御性去重。删除 B1 只删本机；删除 P1 先持久化 marker，再调用 owner-scoped `delete-my-record`，明确受理后级联删除当前设备 B1，失败则保留并在刷新／重启恢复。生产只更新四个直接受影响函数；反向下载的确定性树哈希分别为 `finalize-board-observation=eea84abd934a110db8af6346d470fd883917449b5a63c14a7eda788e89453e7b`、`finalize-draw-update=370fbab9f1a53d1b9c8514de474a633c66c29f7508c0fb7632b57f07f9d5e69a`、`get-my-records=c4ab06aa549a57f4a599e3b0216bc44680a9f44e606a49aafa89775a054ad3e9`、`recognize-draw-tickets=47699adf926ab49a96c190fb6cf8916f3ab43b5fa60b01f037d3b53913a810aa`。

2026-08-28 V1 MY_RECORDS projection / quota blocker：小程序 WXML 的 `local-records` 只循环 `drafts`，云端记录不再进入本机列表；`contributions` 只循环服务端已收敛的 current `cloudClues`。同 Board 的历史 O1/O2/O3 多行临时投影已废止，`refreshCloudRecords()` 只接收／防御性保留一个 current P1。识别入口在 ready account 下以已知零值同步快拒绝；已知可用时复用 `refreshCloudQuota()` 只读预检，避免重复 bootstrap/profile/records，`onShow` 的完整账号刷新与关闭额度耗尽提示后的 quota-only 刷新负责让跨日或管理端重置后的旧零值失效。

2026-08-28 V1 Local Board 删除入口真机回归：`index.wxml` 的 `record-row` swipe 容器、`swipeX` 手势、垃圾桶按钮和 `onDeleteDraft` 一直存在；入口消失来自 `summarizeLocalDrawDraft()` 的旧展示条件 `canDelete = submissionState === local`，使已绑定／已上传 Local Board 在 WXML `wx:if` 被隐藏。Local Board summary 现对所有可读本机 Board 固定 `canDelete=true`，因此 ACTIVE_UNBOUND、ACTIVE_BOUND 与 ACTIVE_STALE_REFERENCE 都可通过原右滑入口明确删除；handler 仍只执行 `draftRepository.delete(boardId)`，不会调用 `delete-my-record`，云端 Observation 继续独立保留。

2026-08-28 历史 lifecycle 说明：早先“同 Board 多 Observation 全展示、云端删除后将 B1 重置为可再次上传”的中间实现已被上方最终架构替代。保留项是 MY_RECORDS 的 repository-only 删除和失败／随机缺失不误删 B1；最终云端显式删除则通过 durable marker 在明确受理后删除当前设备 B1。

2026-08-28 V1 UPLOAD_STUCK_PENDING blocker：服务端正式六位观察记录码契约和生成器允许任意 `^[A-Z0-9]{6}$`，包括全字母与全数字；小程序 `local-draw-drafts.ts` 曾额外要求码中同时出现字母和数字。生产新 Observation 获得合法全字母码后，submit 已写入 `PHOTO_PENDING`，但版本化 Storage decoder 在后台 verify 前把整条 Local Board draft 判为不兼容，`runTicketVerification` 的版本 guard 因找不到 draft 而 early return。客户端 validator 现与正式契约统一；本地确定性派生器仍可继续生成同时含字母和数字的易读码。未改服务端码算法、Observation 身份、LOCATION/PHOTO/NOTE 状态机或 Storage/ACL。

2026-08-28 V1 release blocker 兼容基础：`local-draw-drafts.ts` 从既有 Local Board 重建不可变初始 Cloud snapshot，R2 读取 `initialRemainingTickets/isGrandPrize`，legacy 由当前 remaining 与 history 恢复基线，且不执行 draw side effect。`cloud-recognition-task.ts` 复用 `prepare-new-upload`；该 action 现按最终模型复用 owner+board 的稳定 P1，而不是为已送审记录派生新 Observation。NEW upload 的 `shareNote` 清空，NOTE_FAILED 同一 submission 仍加载其原文。

2026-08-28 V1 release blocker 单函数生产部署：只更新 `finalize-board-observation`，没有部署其余 18 个函数或修改环境变量、触发器、依赖层和运行配置。第一次 CLI 在错误临时工作目录报告成功但反向下载仍是旧包，未被视为完成；改为函数目录内更新后，生产 `$LATEST` 反向下载与 `services/cloudbase/.deploy/functions/finalize-board-observation` 逐文件一致，确定性树哈希均为 `a1671fa2c7f5bb9bec9f90d3627e09429a20318f278ebad913a72e673c2a6a27`，线上为 Nodejs20.19、Active/Available、`index.main`、依赖层 v1。
...
历史执行快照（2026-08-18）：当时唯一活动区块为 V1-F｜跨端质量与小程序发布门（`IN_PROGRESS`）；该状态现已由本文顶部 2026-08-30 closure 取代。
