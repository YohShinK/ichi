...
2026-08-29 V1.0.0 architecture closure：Local Board／Upload Submission／Current Cloud Publication 三层模型及 7 条产品规则均已实现、真机验证并 `FROZEN / CLOSED`，不再是 active design task。P1 保持轻量 published-version pointer，完整 snapshot/events 留在 versioned submission；冻结 R2 三 SHA、Storage CUSTOM、`recognition-temp/ Days=1` 与 13 个私有集合边界不变。当前没有活动区块；V1 唯一活动计划只是微信审核通过后的正式发布与最小线上确认。V1.0.1 和 V2 仅为 backlog。
